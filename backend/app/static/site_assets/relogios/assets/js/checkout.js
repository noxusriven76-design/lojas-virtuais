...
function paymentLabelFromCode(code){
  ...
  if(key === 'credit_card') return 'Cartao de credito';
  if(key === 'debit_card') return 'Cartao de debito';
  if(key === 'boleto') return 'Boleto';
  if(key === 'cash') return 'Dinheiro';
  return key || 'Pagamento';
}

function normalizePaymentMethodForOrder(code){
  const key = String(code || '').trim().toLowerCase();
  if(key === 'credit_card' || key === 'debit_card') return 'card';
  return key;
}

async function fetchStorePaymentMethods(storeSlug){
  const slug = String(storeSlug || '').trim();
  if(!slug) return [];
  const resp = await fetch(`/api/v1/stores/${encodeURIComponent(slug)}/payment-methods`, {
    method: 'GET',
    headers: { 'Accept': 'application/json' },
  });
  if(!resp.ok){
    throw new Error(`Falha ao carregar formas de pagamento (${resp.status}).`);
  }
...
function renderPaymentMethods(container, methods){
  ...
  for(const method of list){
    ...
    if(!code) continue;
    const labelText = String(method?.label || '').trim() || paymentLabelFromCode(code);
  ...
  const paymentInputs = Array.from(container.querySelectorAll('input[name="payment"]'));
...
function validateForm(form){
  ...
  for(const el of fields){
    ...
    if(valid && name === 'cep' && !validateCep(value)){
      valid = false;
      msg = 'Informe um CEP válido (8 dígitos).';
    }
...
function renderShippingOptions(container, options, selectedService, onSelect){
  ...
  if(!list.length){
    ...
    p.className = 'help';
    setText(p, 'Informe um CEP para ver opções de frete.');
...
function renderOrderSummary(container, quote, selectedShipping){
  ...
  setText(small, 'Totais sujeitos à validação do backend.');
...
export function initCheckout(){
  ...
  const paymentMethodsContainer = form.querySelector('[data-payment-methods]');
  const paymentMethodsMsg = form.querySelector('[data-payment-methods-message]');

  async function loadPaymentMethods(){
    if(!paymentMethodsContainer) return;
    try{
      const methods = await fetchStorePaymentMethods(STORE.slug);
      const count = renderPaymentMethods(paymentMethodsContainer, methods);
      if(count === 0){
        if(paymentMethodsMsg) setText(paymentMethodsMsg, 'Nenhuma forma de pagamento ativa para esta loja. Configure no painel.');
        if(submitBtn) submitBtn.disabled = true;
      } else {
        if(paymentMethodsMsg) setText(paymentMethodsMsg, '');
      }
    } catch {
      // fallback: mantem opcoes estaticas quando endpoint nao estiver disponivel
      const paymentInputs = Array.from(form.querySelectorAll('input[name="payment"]'));
      if(paymentInputs.length && !paymentInputs.some(i => i.checked)){
        paymentInputs[0].checked = true;
      }
      if(paymentMethodsMsg) setText(paymentMethodsMsg, 'Usando formas de pagamento locais (fallback).');
    }
  }
  ...
  async function refreshQuote({ showToastOnError=false }={}){
    ...
    if(!validateCep(cep)){
      ...
      setShippingMessage('Não encontramos opções de frete para este CEP.');
    } else {
      setShippingMessage('');
    }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if(validateCep(cepInput ? cepInput.value : '') && !selectedShipping){
      setError(submitErr, 'Selecione uma opção de frete para continuar.');
    ...
    const paymentMethod = normalizePaymentMethodForOrder(pickPaymentMethod(form) || 'pix');
    ...
    try {
      ...
      if(orderId == null){
        throw new Error('Pedido criado, mas não recebemos o número do pedido.');
      }

      // Salva para a página de confirmação
      const discounts = Array.isArray(quote?.discounts) ? quote.discounts : [];
      const summary = {
        orderId,
      ...
      const msg = (err && err.message) ? err.message : 'Não foi possível criar o pedido.';
  ...
  // Primeira carga
  loadPaymentMethods().finally(() => refreshQuote({}));
}

// Mantém compatibilidade com o entry atual
