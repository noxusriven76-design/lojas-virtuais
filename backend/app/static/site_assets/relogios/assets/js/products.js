/*
  CatÃ¡logo local (mock) + carregamento opcional via API.

  Objetivo:
  - Mock NÃƒO Ã© a fonte principal (desativado por padrÃ£o em assets/js/config.js)
  - Quando USE_MOCK_DATA=false, categorias/produtos vÃªm do backend no contexto do STORE_SLUG.

  ObservaÃ§Ã£o:
  - O backend atual nÃ£o possui campos de "slug" no modelo, entÃ£o o front gera slugs
    determinÃ­sticos (slugify(name + '-' + id)) para manter URLs estÃ¡veis.
*/
...
function _toNumber(value){
  ...
  // Accept values like "1999.90", "1.999,90", "1999,90", "R$ 1.999,90"
...
function _shortDescription(desc){
  ...
  if(!d) return 'Produto selecionado â€” confira detalhes e variaÃ§Ãµes disponÃ­veis.';
...
function getPrimaryPrice(product){
  const sale = _toNumber(product?.sale_price);
  const base = _toNumber(product?.base_price ?? product?.price);
  if(Number.isFinite(sale) && sale > 0) return sale;
  if(Number.isFinite(base) && base > 0) return base;

  const variants = Array.isArray(product?.variants) ? product.variants : [];
  const variantPrices = variants
    .map(v => _toNumber(v?.price ?? v?.base_price))
...
// ---------------------------------------------------------------------------
// API: carregar catÃ¡logo real (quando USE_MOCK_DATA=false)
// ---------------------------------------------------------------------------
...
async function _loadCatalogFromApi(){
  ...
  // slug por nome, com fallback determinÃ­stico.
  ...
  const normProducts = allProducts
    ...
    .map(p => {
      ...
      const base_price = _toNumber(p.base_price ?? p.price);
      const sale_price = _toNumber(p.sale_price);
      ...
      const variants = variantsRaw
        ...
        .map(v => ({
          ...
          color: String(v.color || '').trim() || 'PadrÃ£o',
          size: String(v.size || '').trim() || 'Ãšnico',
          price: _toNumber(v.price ?? v.base_price ?? p.base_price ?? p.price),
...
// FunÃ§Ã£o pÃºblica: garante catÃ¡logo disponÃ­vel.
