...
//     POST /checkout/quote
//     POST /orders
// - Se no existirem / offline, usar fallback (mock) com estrutura compatvel.
//
// Quote (mnimo): { items, subtotal, shippingOptions, discounts, total }
// Create order (mnimo): { orderId, paymentInstructions? }
// ---------------------------------------------------------------------------

const LV = (window.LV_CONFIG || LV_CONFIG || { USE_MOCK_DATA: false });

// Reutiliza a mesma chave do mock de pedidos (customerApi) para melhor integrao offline.
...
function _mockTokenUserId(token){
  // Compatvel com o formato do mock em customerApi: mock:{userId}:{ts}:{rand}
...
function _computeMockShippingOptions({ cep, items }){
  ...
  // Replica a lgica simples do backend (shipping.py) para ficar consistente.
...
function _normalizeShippingOptions(any){
  ...
  return list.map((o) => ({
    service: String(o?.service ?? o?.id ?? o?.code ?? '').trim() || 'Padro',
...
function _normalizeQuoteResponse(data, payload){
  ...
  // Se o endpoint no devolve campos essenciais, tenta manter compatibilidade.
  if(enriched && (subtotal > 0 || total > 0 || shippingOptions.length || discounts.length)){
    // Se subtotal no veio, calcula.
    const computedSubtotal = subtotal > 0 ? subtotal : enriched.reduce((s,it)=>s+_money(it.line_total),0);

    // Se total no veio, calcula com base no shipping_service da requisio.
...
function _paymentInstructions({ paymentMethod, orderId, total }){
  ...
  if(m === 'pix'){
    ...
    return `Pague via PIX\n\n Pedido: ${orderId}\n Valor: ${money(total)}\n\nCopia e cola:\n${key}\n\nAps o pagamento, a confirmao pode levar alguns minutos.`;
  }
  ...
  if(m === 'card'){
    return 'Carto (placeholder): seu pagamento ser processado e voc receber a confirmao na tela e por e-mail (se integrado).';
  }
...
function _mockCreateOrder(orderPayload, { token='', paymentMethod='' }={}){
  ...
  // salva um pedido mnimo no mock para aparecer em "Meus pedidos".
  ...
  list.unshift({
    ...
    shipping: {
      method: String(orderPayload?.shipping_service || 'Padro'),
...
export async function createOrder(orderPayload, { token='', paymentMethod='' }={}){
  ...
  try {
    ...
    // Alguns backends podem rejeitar campos extras (422). Tenta novamente apenas com o schema mnimo.
    if(status === 422){
    ...
    // Se o backend no existir / offline, cai no mock.
    if(_shouldFallbackToMock(handledErr)) return _mockCreateOrder(body, { token, paymentMethod });

    // Se no est autenticado e no h token, permite fallback.
