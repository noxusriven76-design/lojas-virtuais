...
export async function initAccountPage(){
  ...
  try {
    ...
    if(!user){
      // Token existe, mas usurio no carregou por algum motivo.
    ...
    if(!u){
      renderEmptyState(root, {
        title: 'No encontramos seus dados',
        message: 'Faa login novamente para continuar.',
        actionText: 'Entrar',
    ...
    setText(p, 'Informaes bsicas da sua conta.');
    ...
    setText(pb, 'Acesse seus pedidos ou finalize sua sesso.');
    ...
    if(alertEl){
      ...
      setText(alertEl, (err && err.message) ? err.message : 'No foi possvel carregar sua conta.');
    }
    renderErrorState(root, {
      title: 'Erro ao carregar sua conta',
      message: 'Tente novamente ou faa login de novo.',
      actionText: 'Entrar',
      actionHref: session.buildLoginHref({ returnTo: 'account.html' }),
    });
  }
