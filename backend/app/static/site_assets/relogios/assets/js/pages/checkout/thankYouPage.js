...
function renderSummary(container, summary){
  ...
  setText(p, id ? `Nmero do pedido: ${id}` : 'Seu pedido foi criado.');
  ...
  if(instr){
    ...
    setText(th, 'Instrues de pagamento');
...
export async function initThankYouPage(){
  ...
  setText(h, 'Carregando confirmao...');
  const p = document.createElement('p');
  ...
  try {
    // Preferncia: sessionStorage (fluxo logo aps checkout)
    ...
    if(orderId){
      ...
      if(!summary){
        throw new Error('No foi possvel carregar o pedido.');
      }
    ...
    renderEmptyState(root, {
      ...
      message: 'No identificamos um pedido para exibir nesta pgina.',
      primaryAction: { label: 'Ir para meus pedidos', href: 'orders.html' },
      secondaryAction: { label: 'Voltar ao catlogo', href: 'category.html' },
    });
  } catch (err){
    const msg = (err && err.message) ? err.message : 'Erro ao carregar confirmao.';
    if(alertEl){
    ...
    renderErrorState(root, {
      title: 'Erro ao carregar confirmao',
      message: msg,
