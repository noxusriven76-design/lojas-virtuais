...
export async function initOrderPage(){
  ...
  try {
    ...
    if(!order){
      renderErrorState(root, {
        title: 'Pedido no encontrado',
        message: 'No conseguimos carregar os dados do pedido.',
        actionText: 'Voltar',
        actionHref: 'orders.html',
      });
      return;
    ...
    ['Produto','Qtd','Preo','Subtotal',''].forEach((c) => {
    ...
    if(alertEl){
      ...
      setText(alertEl, (err && err.message) ? err.message : 'No foi possvel carregar o pedido.');
    }
    renderErrorState(root, {
