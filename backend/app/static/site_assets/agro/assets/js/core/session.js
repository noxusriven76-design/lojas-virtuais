...
export function createSession({ tokenKey=DEFAULT_TOKEN_KEY, userKey=DEFAULT_USER_KEY }={}){
  ...
  function _safeReturnTo(value, fallback='account.html'){
    ...
    // Permite apenas navegação dentro do próprio site (arquivos .html)
  ...
  async function refreshUser(){
    ...
    try {
      ...
      // Se token inválido, limpa sessão.
  ...
  function requireAuth({ redirectTo='login.html', validate=false }={}){
    ...
    if(validate){
      // dispara validação assíncrona (sem bloquear a navegação)
      refreshUser().catch(() => {
        const href = `${redirectTo}?returnTo=${encodeURIComponent(_currentRelativeUrl())}`;
        window.location.replace(href);
      });
  ...
  return {
    ...
    tokenKey,
    userKey,
    // expostos para páginas/depuração
    _stripBearer,
    _safeReturnTo,
    _currentRelativeUrl,
  };
