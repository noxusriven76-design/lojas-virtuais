...
function _showProductEmpty(slug){
  ...
  if(titleEl) titleEl.textContent = `Produto não encontrado | ${STORE.name}`;
}
...
function renderVariantSelectors(product){
  ...
  function update(){
    ...
    const price = Number.isFinite(variantPrice) && variantPrice > 0 ? variantPrice : getPrimaryPrice(product);
...
async function renderProductPage(){
  ...
  // Head meta
  const titleEl = document.querySelector('title');
  if(titleEl) titleEl.textContent = `${product.name} | ${STORE.name}`;
  ...
  setOg('og:title', `${product.name} | ${STORE.name}`);
