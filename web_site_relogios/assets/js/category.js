...
function applyFilters(list, state){
  ...
  if(state.cat === 'novidades'){
    ...
    // sem flags: mantm lista e ordenao "new" d prioridade ao mais recente.
  }
...
function updateBreadcrumbs(state){
  ...
  const parts = [{label:'Incio', href:'index.html'}];
  if(state.cat){
    const catName = getCategoryBySlug(state.cat)?.name || (state.cat === 'novidades' ? 'Novidades' : state.cat === 'promocoes' ? 'Promoes' : 'Categoria');
    ...
    parts.push({label:'Catlogo', href:'category.html'});
  }
  ...
  if(jsonEl){
    ...
    const data = [
      ...
      },
      {
        ...
        "@type": "CollectionPage",
        name: pageName,
        description: "Explore roupas e acessrios com esttica minimalista, com foco em caimento e qualidade.",
...
function renderPagination(total, page, perPage, baseParams){
  ...
  function link(p, label){
    ...
    a.textContent = label;
    a.setAttribute('aria-label', `Ir para pgina ${p}`);
    return a;
  }
  ...
  next.textContent='Prxima';
  next.href = link(Math.min(totalPages, page+1), 'Prxima').href;
...
async function renderCategory(){
  ...
  // Loading state (s na primeira carga real do catlogo)
  ...
  // heading e meta
  const catName = params.cat ? (getCategoryBySlug(params.cat)?.name || (params.cat==='novidades' ? 'Novidades' : params.cat==='promocoes' ? 'Promoes' : 'Catlogo')) : (params.q ? `Resultados para ${params.q}` : 'Catlogo');
  ...
  if(descEl) descEl.setAttribute('content', `Explore ${catName.toLowerCase()} com esttica minimalista. Fotos grandes, bons detalhes e compra rpida com entrega em 24h (placeholder).`);

  setCanonical(`${STORE.url}/category.html`);

  const all = getActiveProducts();
  // Se h fallback local carregado, considere o catlogo "pronto" para evitar flicker em re-renders.
  if(!_catalogReady && all && all.length > 0) _catalogReady = true;
  // Se falhou o catlogo remoto e tambm no h fallback local, exibe erro.
  if(catalogError && (!all || all.length === 0)){
    if(seq !== _renderSeq) return;
    if(grid){
      grid.setAttribute('aria-busy','false');
      renderErrorState(grid, {
        title: 'No foi possvel carregar o catlogo',
  ...
  // popular opes com base no universo
  ...
  if(grid){
    ...
    if(pageItems.length === 0){
      renderEmptyState(grid, {
        ...
        secondaryAction: { label: 'Ver catlogo', href: 'category.html' }
      });
    } else {
      if(catalogError){
