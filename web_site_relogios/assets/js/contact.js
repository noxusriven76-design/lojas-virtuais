...
function validateContact(form){
  ...
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value||'')){ err(email, 'Informe um e-mail valido.'); ok=false; }
  if((message.value||'').trim().length < 10){ err(message, 'Descreva sua duvida com pelo menos 10 caracteres.'); ok=false; }
