...
// - Se a API não existir (offline / 404), ativar fallback "mock" em localStorage.
// - Manter assinatura simples e previsível para páginas (login, conta, pedidos).
// ---------------------------------------------------------------------------
...
function _hashPw(password){
  // Hash super simples (mock). Não use em produção.
...
function _shouldFallbackToMock(err){
  // Só cai no mock quando a API não existe / offline.
  // Se a API existe mas retorna 401, o erro deve aparecer ao usuário.
...
function _mockEnsureSeed(){
  // Seed leve só para deixar o mock utilizável no primeiro uso.
  ...
  const sample = [
    {
      ...
      shipping: { method: 'Padrão', eta_days: 3, address: 'Rua Exemplo, 123 - São Paulo/SP' },
    },
    {
      id: 1002,
      user_id: user.id,
      ...
      currency: 'BRL',
      items: [
        { product_slug: 'camisa-oversized-algodao', name: 'Camisa Oversized Algodão', qty: 2, unit_price: 149.9 },
      ],
      shipping: { method: 'Express', eta_days: 1, address: 'Rua Exemplo, 123 - São Paulo/SP' },
    },
  ];
...
function _mockSignup({ name, email, password }){
  _mockEnsureSeed();
  const e = _normEmail(email);
  if(!e || !e.includes('@')) throw new ApiError('Informe um e-mail válido.', { status: 400, code: 'invalid_email' });
  const users = _getMockUsers();
  if(users.some(x => _normEmail(x.email) === e)){
    throw new ApiError('Este e-mail já está cadastrado.', { status: 409, code: 'email_exists' });
  }
...
function _mockForgotPassword({ email }){
  ...
  // Sempre retorna ok (boa prática para não enumerar e-mails).
  return { ok: true };
}
...
function _mockMe(token){
  ...
  if(!userId) throw new ApiError('Não autenticado.', { status: 401, code: 'unauthorized' });
  const users = _getMockUsers();
  const u = users.find(x => Number(x.id) === Number(userId));
  if(!u) throw new ApiError('Sessão inválida.', { status: 401, code: 'invalid_session' });
...
function _mockOrder(token, id){
  ...
  if(!order) throw new ApiError('Pedido não encontrado.', { status: 404, code: 'order_not_found' });
  return { order };
}
...
export async function getOrder(token, id){
  ...
  if(!safeId) throw new ApiError('Pedido inválido.', { status: 400, code: 'invalid_order_id' });
  try {
