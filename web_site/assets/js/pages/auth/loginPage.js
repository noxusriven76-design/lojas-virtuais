...
export function initLoginPage(){
  // Se já estiver logado, manda para a conta.
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if(status === 401 || status === 403){
        showAlert(alertEl, { kind: 'error', message: 'E-mail ou senha inválidos.' });
      } else {
        showAlert(alertEl, { kind: 'error', message: (err && err.message) ? err.message : 'Não foi possível entrar agora.' });
      }
    } finally {
      if(submit) {
